...
function init() {
    ...
    // TODO: keyboard cotrols
    // document.addEventListener('keydown', function(event) {
    //     if (event.code == "KeyW" ) cameraPosition.y += 0.25
    //     if (event.code == "KeyS" ) cameraPosition.y -= 0.25
    //     if (event.code == "KeyD" ) cameraPosition.x += 0.25
    //     if (event.code == "KeyA" ) cameraPosition.x -= 0.25
    // });
...
document.addEventListener('keydown', function(event) {
    if ( event.code == "Enter" && modals.create.isShow ) {
        createSendForm()
    }
});
...
function onDocumentMouceClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (INTERSECTED.name == "Dot" && mode == "delete") {
            $.ajax({
                type: "POST",
...
function onDocumentMouseDown(event) {
    ...
    // if( mode == "move" ) {
        ...
        if (intersects.length > 0) {
            ...
            // console.log("draggingDot:", draggingDot)
...
function onDocumentMouseUp(event) {
    ...
    // if( mode == "move" ) {
        ...
        if (intersects.length > 0) {
            INTERSECTED = intersects[0].object
            // if (INTERSECTED.name == "Earth" && mode == "move" && draggingDot) {
